...
class VideoAbstract(abc.ABC):
    ...
    def download(
        ...
        **kwargs
        ...
        if stream:
            self._write(stream, workers=workers, progress_callback=progress_callback, *args, **kwargs)
        else:
            with open(self._build_file_path(path), 'wb') as file:
                self._write(file, workers=workers, progress_callback=progress_callback, *args, **kwargs)
...
class RutubeVideo(VideoAbstract):
    ...
    def _get_segment_data(uri):
        ...
        while retry:
            try:
                r = requests.get(uri, timeout=(10, 30))  # 10 сек на подключение, 30 на чтение
                if r.status_code == 200:
                    return r
            except requests.exceptions.Timeout:
                logger.warning(f"Timeout while downloading segment {uri}")
            except requests.exceptions.RequestException as e:
                logger.warning(f"Error downloading segment {uri}: {str(e)}")
    ...
    def _write_threads(self, bar, stream: Optional[BinaryIO], workers: int = 0, progress_callback=None):
    ...
    def _write(self, stream: Optional[BinaryIO], workers: int = 0, progress_callback=None, *args, **kwargs):
        total_segments = len(self._get_segment_urls())
        if total_segments == 0:
            return
...
class Rutube:
    ...
    def _get_m3u8_url(self):
        try:
            ...
            print('Ошибка: ключ video_balancer отсутствует в ответе от API')
